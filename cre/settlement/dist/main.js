import { EVMClient, HTTPClient, getNetwork, logTriggerConfig, prepareReportRequest, bytesToHex, blockNumber, encodeCallMsg, protoBigIntToBigint, CronCapability, LAST_FINALIZED_BLOCK_NUMBER, TxStatus, handler, Runner, json, ok, ConsensusAggregationByFields, median, identical, ignore, } from "@chainlink/cre-sdk";
import { decodeEventLog, decodeFunctionResult, encodeAbiParameters, encodeFunctionData, parseAbiParameters, keccak256, toEventSelector, toHex, } from "viem";
import { z } from "zod";
const OUTCOME_YES = 1;
const OUTCOME_NO = 2;
...
const SETTLEMENT_REQUESTED_TOPIC = toEventSelector("SettlementRequested(uint256,string,uint32,uint16)");
...
const CRYPTO_SETTLEMENT_REQUESTED_TOPIC = toEventSelector("SettlementRequested(uint256,uint8,uint64,uint64)");
...
const STOCK_SETTLEMENT_REQUESTED_TOPIC = toEventSelector("SettlementRequested(uint256,address,uint64,uint64,uint64,uint32)");
...
const RESERVE_SETTLEMENT_REQUESTED_TOPIC = toEventSelector("SettlementRequested(uint256,address,uint64,uint64,uint32)");
...
export const newReadBudget = (reserved = 0) => ({ left: CHAIN_READ_LIMIT - reserved });
/**
 ...
 * a string can.
 */
export const parseAeroDataBoxUtc = (s) => {
    const m = /^(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2})Z$/.exec(s);
    if (!m)
        throw new Error(`Unrecognized AeroDataBox timestamp: ${s}`);
    const [, y, mo, d, h, mi] = m;
...
const readAeroDataBox = (sendRequester, apiUrl, apiKey, flightIata, departureDateIso) => {
...
const readSecondary = (sendRequester, url, flightIata, departureDateIso) => {
    const response = sendRequester
        .sendRequest({
    ...
    return {
        delayMinutes: parsed.arrivalDelayMinutes === null ? 0 : Math.round(parsed.arrivalDelayMinutes),
 ...
 */
export const outcomeFor = (reading, thresholdMinutes) => {
    if (reading.status === "cancelled")
        return OUTCOME_YES;
    if (reading.status !== "landed")
        return OUTCOME_VOID;
    return reading.delayMinutes >= thresholdMinutes ? OUTCOME_YES : OUTCOME_NO;
};
const fetchFlight = (sendRequester, apiUrl, apiKey, secondaryUrl, flightIata, departureDateIso, thresholdMinutes) => {
    const readings = [
    ...
    if (!allAgree) {
        ...
        // dressed up as data.
        throw new Error(`Sources disagree on outcome for ${flightIata}: ${readings
    ...
    const medianDelay = sorted.length % 2 === 1
...
const requireOk = (response, venue) => {
    if (!ok(response)) {
 ...
 */
const readCoinbasePrice = (sendRequester, symbol, minuteStart) => {
    ...
    if (!candle)
        throw new Error(`Coinbase has no candle for minute ${minuteStart}`);
    return candle[4];
};
const readBitstampPrice = (sendRequester, symbol, minuteStart) => {
...
const readKrakenPrice = (sendRequester, symbol, minuteStart) => {
    ...
    for (const key of Object.keys(parsed.result)) {
        if (key !== "last" && Array.isArray(parsed.result[key])) {
            candles = parsed.result[key];
            break;
        }
    }
    if (!candles)
        throw new Error("Kraken returned no candle series");
    for (const row of candles) {
        const c = row;
        if (Number(c[0]) === minuteStart)
            return Number(c[4]);
    }
...
export const toScaledPrice = (price) => Math.round(price * PRICE_DECIMALS);
/**
 ...
 */
const MIN_VENUES = 2;
export const reconcileVenuePrices = (names, prices, strikePrice, symbol) => {
    if (prices.length < MIN_VENUES) {
        throw new Error(`Only ${prices.length} venue(s) priced ${symbol}, need ${MIN_VENUES}` +
    ...
    if (!outcomes.every((o) => o === outcomes[0])) {
        throw new Error(`Venues disagree on ${symbol} vs strike ${strikePrice}: ${names
...
const fetchCryptoPrice = (sendRequester, symbol, minuteStart, strikePrice) => {
    const venues = [
     ...
     */
    if (prices.length < MIN_VENUES) {
        throw new Error(`Only ${prices.length} of ${venues.length} venues priced ${symbol} at minute ` +
...
const settleFlightMarket = (runtime, t) => {
    const { marketId, flightIata, departureDate, thresholdMinutes } = t;
    runtime.log(`Settling market ${marketId}: ${flightIata} on ${departureDate}, threshold ${thresholdMinutes}m`);
    ...
    const aggregation = ConsensusAggregationByFields({
        delayMinutes: median,
        status: identical,
        fetchedAt: ignore,
    });
    ...
    try {
        const obs = httpClient
            .sendRequest(runtime, fetchFlight, aggregation)(runtime.config.apiUrl, resolveApiKey(runtime), runtime.config.secondaryUrl ?? "", flightIata, departureDateIso, Number(thresholdMinutes))
            .result();
        ...
        outcome = outcomeFor({ delayMinutes: obs.delayMinutes, status: obs.status }, Number(thresholdMinutes));
    }
    catch (err) {
    ...
    runtime.log(`Market ${marketId} -> outcome=${outcome} delay=${observedDelay}m status=${observedStatus} evidence=${evidenceHash}`);
    ...
    const encoded = encodeAbiParameters(parseAbiParameters("uint256 marketId, uint8 outcome, int32 delayMinutes, bytes32 evidenceHash"), [marketId, abiInt(outcome), abiInt(observedDelay), evidenceHash]);
...
export const onSettlementRequested = (runtime, triggerEvent) => {
    ...
    const decoded = decodeEventLog({
        ...
        topics: triggerEvent.topics.map((t) => bytesToHex(t)),
    });
...
const settleCryptoMarket = (runtime, t) => {
    ...
    runtime.log(`Settling crypto market ${marketId}: ${symbol} vs strike ${strike} at minute ${minuteStart}`);
    const httpClient = new HTTPClient();
    const aggregation = ConsensusAggregationByFields({
        delayMinutes: median,
        status: identical,
    ...
    const evidenceHash = keccak256(toHex(evidence));
    runtime.log(`Crypto market ${marketId} -> outcome=${outcome} price=${observedPrice} strike=${strike} evidence=${evidenceHash}`);
    const network = getNetwork({
        chainFamily: "evm",
        chainSelectorName: runtime.config.chainSelectorName,
    });
    ...
    const encoded = encodeAbiParameters(parseAbiParameters("uint256 marketId, uint8 outcome, int256 observedValue, bytes32 evidenceHash"), [marketId, abiInt(outcome), BigInt(observedPrice), evidenceHash]);
    const signedReport = runtime.report(prepareReportRequest(encoded)).result();
    const txResult = evmClient
        .writeReport(runtime, {
        receiver,
...
export const onCryptoSettlementRequested = (runtime, triggerEvent) => {
    const decoded = decodeEventLog({
 ...
 */
const ethCall = (runtime, evmClient, to, data, atBlock, budget) => {
    if (budget.left <= 0) {
...
const readFeedRound = (runtime, evmClient, feed, atBlock, budget, roundId) => {
    const data = roundId === undefined
    ...
    if (updatedAt === 0)
        throw new Error(`Feed round ${roundId ?? "latest"} is unset`);
 ...
 */
export const walkToRoundInForce = (read, from, target, maxWalk = MAX_ROUND_WALK) => {
    let round = from;
...
const roundInForceAt = (runtime, evmClient, feed, atBlock, budget, from, target) => walkToRoundInForce((roundId) => readFeedRound(runtime, evmClient, feed, atBlock, budget, roundId), from, target);
/**
 ...
 */
export const checkRoundUsable = (atExpiry, expiry, maxStaleness) => {
 ...
 */
export const normalizeToEightDecimals = (answer, decimals) => {
...
export const resolveStockOutcome = (atExpiry, atClose, strike, decimals) => {
    ...
    if (atClose.answer === atExpiry.answer) {
        throw new Error(`Price did not move between close and expiry (${atExpiry.answer}) — market was already decided`);
    }
...
const settleStockMarket = (runtime, t) => {
    ...
    runtime.log(`Settling stock market ${marketId}: feed ${feed} vs strike ${strike} at ${expiry}, pinned to block ${atBlock}`);
    const network = getNetwork({
        chainFamily: "evm",
        chainSelectorName: runtime.config.chainSelectorName,
    });
    ...
    try {
        const decimals = decodeFunctionResult({
            ...
            data: ethCall(runtime, evmClient, feed, encodeFunctionData({ abi: feedAbi, functionName: "decimals" }), atBlock, budget),
        });
    ...
    const evidenceHash = keccak256(toHex(evidence));
    runtime.log(`Stock market ${marketId} -> outcome=${outcome} price=${observedPrice} strike=${strike} evidence=${evidenceHash}`);
    const encoded = encodeAbiParameters(parseAbiParameters("uint256 marketId, uint8 outcome, int256 observedValue, bytes32 evidenceHash"), [marketId, abiInt(outcome), observedPrice, evidenceHash]);
    const signedReport = runtime.report(prepareReportRequest(encoded)).result();
    const txResult = evmClient
        .writeReport(runtime, {
...
export const onStockSettlementRequested = (runtime, triggerEvent) => {
    const decoded = decodeEventLog({
...
const readMarketCount = (runtime, evmClient, address, atBlock, reads) => {
    const decoded = decodeFunctionResult({
        abi: marketCountAbi,
        functionName: "marketCount",
        data: ethCall(runtime, evmClient, address, encodeFunctionData({ abi: marketCountAbi, functionName: "marketCount" }), atBlock, reads),
    });
 ...
 */
export const idsToScan = (count, window) => {
    const ids = [];
    for (let i = count - 1; i >= 0 && ids.length < window; i--)
        ids.push(BigInt(i));
    return ids;
};
/** The finalized block, and the reads left after paying for it. */
const finalizedBlock = (runtime, evmClient, reads) => {
    if (reads.left <= 0)
        throw new Error("Out of chain reads before resolving a block");
    reads.left--;
 ...
 */
export const onSweepFlights = (runtime) => {
    ...
    for (const id of idsToScan(count, reads.left)) {
        const m = decodeFunctionResult({
            ...
            data: ethCall(runtime, evmClient, address, encodeFunctionData({ abi: flightMarketsAbi, functionName: "markets", args: [id] }), atBlock, reads),
        });
        if (Number(m[6]) !== STATUS_SETTLEMENT_REQUESTED)
            continue;
        ...
        settled++;
        if (settled >= MAX_SETTLEMENTS_PER_SWEEP)
            break;
    }
 ...
 */
export const onSweepCrypto = (runtime) => {
    const address = runtime.config.cryptoContractAddress ?? "";
    if (address === "")
        return "no crypto contract configured";
    const evmClient = sweepEvm(runtime);
    const reads = newReadBudget();
    const atBlock = finalizedBlock(runtime, evmClient, reads);
    const count = readMarketCount(runtime, evmClient, address, atBlock, reads);
    ...
    for (const id of idsToScan(count, window)) {
        const c = decodeFunctionResult({
            abi: coreAbi,
            functionName: "core",
            data: ethCall(runtime, evmClient, address, encodeFunctionData({ abi: coreAbi, functionName: "core", args: [id] }), atBlock, reads),
        });
        if (Number(c[3]) !== STATUS_SETTLEMENT_REQUESTED)
            continue;
        const t = decodeFunctionResult({
            abi: cryptoTermsAbi,
            functionName: "terms",
            data: ethCall(runtime, evmClient, address, encodeFunctionData({ abi: cryptoTermsAbi, functionName: "terms", args: [id] }), atBlock, reads),
        });
 ...
 */
export const onSweepStocks = (runtime) => {
    const address = runtime.config.stockContractAddress ?? "";
    if (address === "")
        return "no stock contract configured";
    const evmClient = sweepEvm(runtime);
    const reads = newReadBudget();
    const atBlock = finalizedBlock(runtime, evmClient, reads);
    const count = readMarketCount(runtime, evmClient, address, atBlock, reads);
    ...
    for (const id of idsToScan(count, window)) {
        ...
        const t = decodeFunctionResult({
            abi: stockTermsAbi,
            functionName: "terms",
            data: ethCall(runtime, evmClient, address, encodeFunctionData({ abi: stockTermsAbi, functionName: "terms", args: [id] }), atBlock, reads),
        });
...
const settleReserveMarket = (runtime, t) => {
    const { marketId, feed, maxStaleness, atBlock, budget } = t;
    const strike = t.strikePrice;
    const expiry = t.expiryTime;
    runtime.log(`Settling reserve market ${marketId}: feed ${feed} vs strike ${strike} at ${expiry}, pinned to block ${atBlock}`);
    const network = getNetwork({
        chainFamily: "evm",
        chainSelectorName: runtime.config.chainSelectorName,
    });
    ...
    const evidenceHash = keccak256(toHex(evidence));
    runtime.log(`Reserve market ${marketId} -> outcome=${outcome} level=${observedLevel} strike=${strike} evidence=${evidenceHash}`);
    const encoded = encodeAbiParameters(parseAbiParameters("uint256 marketId, uint8 outcome, int256 observedValue, bytes32 evidenceHash"), [marketId, abiInt(outcome), observedLevel, evidenceHash]);
    const signedReport = runtime.report(prepareReportRequest(encoded)).result();
    const txResult = evmClient
        .writeReport(runtime, {
...
export const onReserveSettlementRequested = (runtime, triggerEvent) => {
    const decoded = decodeEventLog({
...
export const initWorkflow = (config) => {
    const network = getNetwork({
        chainFamily: "evm",
    ...
    const handlers = [
        ...
        evmClient.logTrigger(logTriggerConfig({
            addresses: [config.flightContractAddress],
    ...
    if (config.cryptoContractAddress && config.cryptoContractAddress !== "") {
        handlers.push(handler(evmClient.logTrigger(logTriggerConfig({
            addresses: [
                config.cryptoContractAddress,
                ...(config.ammContractAddress && config.ammContractAddress !== ""
                    ? [config.ammContractAddress]
                    : []),
            ],
    ...
    if (config.stockContractAddress && config.stockContractAddress !== "") {
        handlers.push(handler(evmClient.logTrigger(logTriggerConfig({
            addresses: [config.stockContractAddress],
    ...
    if (config.reserveContractAddress && config.reserveContractAddress !== "") {
        handlers.push(handler(evmClient.logTrigger(logTriggerConfig({
            addresses: [config.reserveContractAddress],
    ...
    if (config.sweepSchedule && config.sweepSchedule !== "") {
        ...
        const sweep = (address, fn) => {
            if (!address || address === "" || address === ZERO_ADDRESS)
                return;
